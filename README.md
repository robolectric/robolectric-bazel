...
this selection through aliases into the generated `robolectric_config` repository.
Dependency modules' configuration is ignored so the root controls the selection. An empty list includes no Android
...
Use distinct names to install several independent runtime sets in the same module:

```python
robolectric.configure(
    name = "android_14",
    versions = ["14-robolectric-10818077-i7"],
)
robolectric.configure(
    name = "android_14_and_15",
    versions = [
        "14-robolectric-10818077-i7",
        "15-robolectric-13954326-i7",
    ],
)
use_repo(robolectric, "android_14", "android_14_and_15")
```

Each generated repository contains its own `android-all` Java library and
`properties` file. Select both together in an `android_local_test`:

```python
android_local_test(
    name = "Android14Test",
    srcs = ["Android14Test.java"],
    test_class = "com.example.Android14Test",
    robolectric_properties_file = "@android_14//:properties",
    deps = [
        "@maven//:org_robolectric_robolectric",
        "@android_14//:android-all",
    ],
)
```

Named configurations do not change the default. Omitting `name` configures
`robolectric_config`, which backs the existing labels; without that configuration,
the default still contains every available runtime. Names must be unique and
must not match an existing runtime download repository. Shared runtimes use the
same download repositories across configurations.
...
with the same meaning, plus `configurations = {"name": [versions...]}` for
additional runtime repositories; its no-argument behavior is unchanged.
